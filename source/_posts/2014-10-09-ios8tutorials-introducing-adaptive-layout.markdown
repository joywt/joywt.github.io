...
历史证明美好的时光总是不能持久；苹果推出 iPad.构建布局开始变得笨拙。你有一个选择：创建两个NIBS - 一个 iPhone,一个 iPad。
 ...
 ![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_01.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_02.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_03.png)
现在你将构建一个更新多个城市天气的app - 通过这个过程来增强你的 UIKit 里自适应方面的知识。
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_04.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_05.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_06.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_07.png)

一旦你掌握了这些，下一步就可以创建你自己的演示控制器，用绚丽的动画来实现。

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/01_08.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_01.png)

选择 **iOS\Application\Single View Application**,点击 **Next:**
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_02.png)

设置 **Product Name** 为 **AdaptiveWeather,Language** 为 Swift 并且确保 **Devices** 设置为 **Universal**，像下面一样：
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_03.png)

打开程序，看一看 **Project Inspector** 这里只有一个故事板文件，像下面截图一样：
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_04.png)

**Main.storyboard** 是单一的故事板对所有设备，无论设备的屏幕尺寸。打开故事板你将看到它包含一个 view Controller，但是是一个不熟悉的尺寸：
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_05.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_06.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_07.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_08.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_09.png)

上面添加的约束确保图片视图和头部有一个确定的尺寸间距并且处于左边到右边的中间。现在你需要配置图片视图和文本容器视图之间的间距。**Ctrl-drag**从图片视图向下到容器视图，像这样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_10.png)

再一次展示出约束的上下文菜单。选择 **Vetical Spacing** 像下面这样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_11.png)

这个约束定义了图片视图底部和文本容器视图头部上下间距的量。

**Size Inspector** 在 Xcode 6 中看起来有点不同。选择图片视图打开 **Size Inspector** 看它现在看起来是什么样的：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_12.png)

你将看到你刚添加的三个约束；你可以容易的配置每一个约束从尺寸编辑器的内部。按下 **Buttom Space To:TextContainer** 中的 **Edit** 按钮；你会看到一个对话框来配置约束属性。设置 **Constant** 等于 **20** 像下面：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_13.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_14.png)

点击 **Add 3 constraints** 添加新的约束到你的视图上。这个别针文本容器在视图控制器视图上的左、右、下三边。

你的故事板将看起来像下面截图这样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_15.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_16.png)

双击详情视图左上方的标题 **Image** 重命名为 **cloud**，像下面一样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_17.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_18.png)

由于图片是白色背景透明，在 asset 库中不容易看到。然而，你可以选中 **1x** 格子按下 **space** 快速预览：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_19.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_20.png)

你的故事板现在看起来像下面这样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_21.png)

这是你的图片 - 但是橙色的约束提醒这你还有一点工作要做。首先选择视图控制器的视图，选择底部栏中的 **Resolve Auto Layout Issues** 图标。最后，选择对话框中的 **All Views\Update Frames** 如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_22.png)

视图控制器解决不满足约束条件的视图后，你的故事板看起来像这样：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_23.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_24.png)

屏幕分厂了两个窗格。在 **Jump Bar** 点击 **Automatic** 并选择下拉菜单中的 **Preview**。最中选择 **Main.storyboard** ，像这样:

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_25.png)

助理编辑器中的故事板现在展示的是 4-inch iPhone 的屏幕，如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_26.png)

点击下面预览名称（**iPhone 4-inch**）旁边的面板可预览旋转效果。将展示横屏效果，如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_27.png)

这是个巨大的进步对于多个模拟器上 - 等等!这里还有更多！点击左下角的 **+** 按钮：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_28.png)

选择 **iPhone 5.5-inch** 和 **iPad** 添加多个预览如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_29.png)

注意横屏预览中的问题了吗？cloud 图片太大了。解决这个需要给图片视图添加新的约束。

回到故事板中。从图片视图 **Ctrl-drag** 到视图控制器视图添加新约束。从上下文菜单中选择 **Equal Heights** 如下：
 
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_30.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_31.png)

下一步，选择 **Relation** 设置为 **Less Than or Equal** 设置 **Multiplier** 为 **0.40** 如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_32.png)

这表示cloud 图片会是图片的实际大小或者屏幕高度的40%，哪个小是那个。

你会发现当你更新约束后，预览面板自动更新了。如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_33.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_34.png)

选择上面的 label 用 **Align** 和 **Pin** 菜单设置 label 横向居中，并设置头部距最邻近视图的间距为 10 ，如下面所示：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_35.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_36.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_37.png)

labels 的文字重叠在了一起，不是你想要的效果。然而，不做任何修改它在 iPad 上显示是正确的：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_38.png)

可见，对于 iPhone 文本的字体太大：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_39.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_40.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_41.png)

立即发现在编辑器中有两个不同：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_42.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_43.png)

点击选择 **Align Center X to: Superview** ，然后按下 **Delete** 卸载这个约束在当前的大小类中。这个约束立即从故事板中消失并且变成灰色：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_44.png)

	注意：你需要切换 **This Size Class** 为 **All**，才能看到这个卸载的约束。
	
双击卸载的约束来选择这个约束。底部有一个额外的行，如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_45.png)

这表明这个约束安装在基本布局中，但是不作用与 **Any Width | Compact Height**布局 - 也就是，当前编辑的这个布局。

重复同样的操作来卸载其他左右与图片视图上的三个约束。你的文档大纲和图片视图的尺寸编辑器看起来如下图：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_46.png)

现在你可以为这个大小类添加所需要的约束。用 **Align** 和 **Pin** 菜单设置纵向居中，并设置左边距最邻近视图的间距为 10：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_47.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_48.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_49.png)

菜单显示在选中的约束旁边。Shift-click Top Space to Top Layout Guide and Equal Widths 如下：

![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_50.png)
...
![Smaller icon](http://7sbygq.com1.z0.glb.clouddn.com/image/ios8/02_51.png)
